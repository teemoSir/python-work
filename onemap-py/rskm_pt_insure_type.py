...
def getdata():
    ...
    action = "/api/syncinsurance/getinstype"
    ...
    if response.status_code:
        # print(response.text)
        global pgsql_bj
        pgsql_bj = json.loads(response.text)
...
def connPgInstall():
    ...
    try:
        ...
        cur = conn.cursor()
        sql = "INSERT INTO public.rskm_pt_insure_type(codenum, xzname, description, memo, tablealias, insurancetype, insurancetypeid, order_index) VALUES (%s, %s, %s, %s, %s, %s, %s, %s);"
        print("入库内容")
        for item in pgsql_bj:
            print(item)
            cur.execute(
                sql,
                (
...
def write_json(text):
    ...
    # 构建文件名
    filename = f"data/type_{now.year}_{now.month}_{now.day}.json"
